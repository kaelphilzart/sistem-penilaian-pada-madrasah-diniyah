...
document.addEventListener('DOMContentLoaded', function () {
    ...
    @if(isset($dataJadwal))
    const hariDropdownEdit = document.getElementById('hari{{ $dataJadwal->id }}');
    const guruPiketDropdownEdit = document.getElementById('id_guruPiket{{ $dataJadwal->id }}');

    hariDropdownEdit.addEventListener('change', function () {
        const selectedHari = this.value;
        const id_kelas = document.getElementById('id_kelas').value;
        const tahun_id = document.getElementById('tahun_dropdown').value;

        if (selectedHari !== '0') {
            fetchGuruPiketByHariEdit(selectedHari, tahun_id, {{ $dataJadwal->id }});
        }
    });

    function fetchGuruPiketByHariEdit(hari, tahun_id, jadwalId) {
        fetch('/admin/guru-piket-by-hari', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
        ...
        .then(response => response.json())
        .then(data => {
            guruPiketDropdownEdit.innerHTML = '<option value="0">Tidak Ada</option>';
            data.forEach(guru => {
                const option = document.createElement('option');
                option.value = guru.id;
                option.textContent = guru.nama_guru;
                guruPiketDropdownEdit.appendChild(option);
            });

            const currentGuruPiketId = {{ $dataJadwal->id_guruPiket }};
            if (currentGuruPiketId) {
                guruPiketDropdownEdit.value = currentGuruPiketId;
            }
        })
        .catch(error => console.error('Error:', error));
    }

    $('#edit-jadwal{{ $dataJadwal->id }}').on('show.bs.modal', function () {
        const selectedHari = hariDropdownEdit.value;
        if (selectedHari !== '0') {
            fetchGuruPiketByHariEdit(selectedHari, {{ $dataJadwal->id_ta }}, {{ $dataJadwal->id }});
        }
    });
    @endif
});
